...
export function renderInvitationEmail(input: InvitationEmailInput): {
  ...
  const subject = `Einladung zu NLC Burgdorf WorshipCenter`;

  const text = [
    `Hallo,`,
    ``,
    `${inviterName} hat dich als ${roleLabel} zu NLC Burgdorf WorshipCenter eingeladen.`,
    ``,
    ...
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border:1px solid #e5e5e5;border-radius:16px;overflow:hidden;font-family:Helvetica,Arial,sans-serif;">
      ...
      <tr><td style="padding:32px 32px 8px 32px;">
        ...
        <p style="margin:0 0 24px 0;font-size:15px;line-height:1.6;color:#6b6b6b;">
          <strong style="color:#16181d;">${escape(inviterName)}</strong> hat dich als <strong style="color:#16181d;">${escape(roleLabel)}</strong> zu NLC Burgdorf WorshipCenter eingeladen. Nimm die Einladung an, um dein Konto einzurichten.
        </p>
